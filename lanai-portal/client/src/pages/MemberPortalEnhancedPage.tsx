        ...
        <>
          {ffNumbers && ffNumbers.length > 0 ? (
            <div className="space-y-2">
              {ffNumbers.map(
                (ff: { airline: string; number: string }, i: number) => (
                  <div
                    key={i}
            ...
            <p className="text-xs text-muted-foreground">
              No frequent flyer numbers added
            </p>
          )}
          {adding ? (
            <div className="space-y-2 p-3 bg-muted/20 rounded-lg">
              <div className="grid grid-cols-3 gap-2">
                <Input
                  placeholder="Program (e.g. BA Executive Club)"
                  value={program}
...
function FamilySection({ memberId }: { memberId: number }) {
  ...
  const relIcons: Record<string, React.ElementType> = {
    spouse: Heart,
    partner: Heart,
    child: Baby,
    parent: User,
                      ...
                      <div>
                        ...
                        {fm.dateOfBirth && (
                          <div className="text-xs text-muted-foreground">
                            DOB:{" "}
                            {new Date(fm.dateOfBirth).toLocaleDateString(
                              "en-GB",
                            )}
                          </div>
                        )}
        ...
        invoices.map((inv) => (
          ...
          >
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                {(inv as { description?: string | null }).description}
              </div>
              <div className="text-xs text-muted-foreground">
                Due:{" "}
                {inv.dueDate
...
export default function MemberPortalEnhancedPage() {
  ...
  const handleSave = () => {
    updateProfile.mutate({
      ...
      roomPreferences: roomPreferences ? { notes: roomPreferences } : undefined,
      preferredPaymentMethod: preferredPayment || undefined,
      securityLevel: securityLevel as
        "standard" | "enhanced" | "maximum" | undefined,
  ...
  const startEditing = () => {
    if (profile) {
      const p = profile as {
        ...
        securityPrivacyLevel?: string | null;
        personalAssistantContact?: string | null;
        familyOfficeContact?: string | null;
      ...
      setRoomPreferences(
        typeof p.roomPreferences === "string"
          ? p.roomPreferences
          : p.roomPreferences
            ? JSON.stringify(p.roomPreferences)
            : "",
      );
    }
    setEditing(true);
  ...
  const p = profile as {
    ...
    anniversaryDate?: string | null;
    conciergeNotes?: string | null;
    favouriteDestinations?: string[] | null;
    roomPreferences?: unknown;
    ...
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      ...
      <Tabs defaultValue="profile">
        ...
        <TabsContent value="profile" className="mt-6 space-y-6">
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <Section title="Personal Details" icon={User}>
              <FieldRow
                label="Date of Birth"
                value={editing ? dateOfBirth : (p?.dateOfBirth ?? "")}
                editing={editing}
              ...
              <FieldRow
                ...
                value={editing ? anniversaryDate : (p?.anniversaryDate ?? "")}
                editing={editing}
              ...
              <FieldRow
                ...
                value={editing ? passportExpiry : (p?.passportExpiry ?? "")}
                editing={editing}
              ...
              <FieldRow
                ...
                value={
                  editing ? dietaryRequirements : (p?.dietaryRequirements ?? "")
                }
                editing={editing}
            ...
            <Section title="Travel Preferences" icon={Plane}>
              ...
              <FieldRow
                ...
                editing={editing}
                onChange={setRoomPreferences}
                placeholder="e.g. High floor, King bed, Sea view"
              />
              <div className="py-2">
                <div className="text-xs text-muted-foreground mb-1.5">
            ...
            <Section title="Payment & Security" icon={Shield}>
              <div className="py-2 border-b border-border/30">
                ...
                {editing ? (
                  ...
                  <div className="text-sm font-medium capitalize">
                    {p?.preferredPaymentMethod?.replace("_", " ") ?? (
                      <span className="text-muted-foreground/50 italic">
                        Not set
                      </span>
                    )}
            ...
            <Section title="Personal Contacts" icon={Briefcase}>
              <FieldRow
                ...
                value={
                  editing
                    ? personalAssistant
                    : (p?.personalAssistantContact ?? "")
                }
                editing={editing}
                onChange={setPersonalAssistant}
                placeholder="Name, email or phone"
              />
              <FieldRow
                label="Family Office Contact"
                value={
                  editing ? familyOfficeContact : (p?.familyOfficeContact ?? "")
                }
                editing={editing}
                onChange={setFamilyOfficeContact}
                placeholder="Name, email or phone"
              />
            </Section>
          </div>
          ...
          <Section title="Concierge Notes" icon={Edit3}>
            {editing ? (
              <Textarea
                ...
                placeholder="Internal notes for the concierge team..."
                className="min-h-24"
              />
            ) : (
        ...
        <TabsContent value="ai" className="mt-6 -mx-6 lg:-mx-8">
          <AiConciergePage memberId={memberId} mode="member" />
        </TabsContent>
      </Tabs>
    </div>
  );
