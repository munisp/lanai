...
function formatOperationalCurrency(value: string | number | null | undefined) {
...
export default function Dashboard() {
  ...
  } = trpc.revenueAnalytics.operationalSnapshot.useQuery({ days: 30 });
  ...
  const statCards: StatCard[] = [
    {
      label: "Daily Revenue",
      value: operationalLoading
        ? "…"
        : formatOperationalCurrency(operational?.totalDailyRevenue),
      note: "paid client invoices",
      icon: DollarSign,
    ...
    },
    {
      ...
      value: operationalLoading
        ? "…"
        : formatOperationalCurrency(operational?.averageBookingValue),
      note: "last 30 days",
      icon: Plane,
    ...
    },
    {
      ...
      value: operationalLoading
        ? "…"
        : formatOperationalCurrency(operational?.membershipFeesCollected),
    ...
    },
    {
      ...
      value: operationalLoading
        ? "…"
        : (operational?.activeRequestsCount ?? "—"),
      note: "platform queue",
      icon: Users,
      color: "text-teal-600",
    },
    {
      label: "Open Concierge Tasks",
      value: operationalLoading ? "…" : (operational?.openTasksCount ?? "—"),
    ...
    <div className="p-6 lg:p-8 space-y-8 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {operationalLoading ? (
              "Calculating live platform operations…"
            ) : error ? (
              <span className="text-red-500">CRM error — {error}</span>
            ) : lastRefresh ? (
              `Live platform data · refreshed ${timeAgo(lastRefresh.toISOString())}`
            ) : (
              "Here is your Lanai overview for today."
            )}
          </p>
        </div>
      ...
      {error && (
        <div className="flex items-center gap-3 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
          ...
          <span>
            CRM connection error. Ensure Twenty CRM is running on port 3000.{" "}
      ...
      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 stagger-children">
        {statCards.map((stat) => (
          <div key={stat.label} className="lanai-card p-5 animate-fade-in-up">
            ...
            <div
              className={cn(
                "text-2xl font-bold text-foreground mb-0.5",
      ...
      <section className="lanai-card p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Paid client invoice line items over the last 30 days.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {[
            ["Hotels", operational?.revenueByCategory.hotels],
            ["Ancillary", operational?.revenueByCategory.ancillary],
            [
              "Luxury Transport",
              operational?.revenueByCategory.luxuryTransport,
            ],
            ["Villas", operational?.revenueByCategory.villas],
            ["Apartments", operational?.revenueByCategory.apartments],
          ].map(([label, value]) => (
            <div key={label as string} className="rounded-lg bg-muted/30 p-3">
              ...
              <div className="mt-1 font-semibold">
                {operationalLoading
                  ? "…"
                  : formatOperationalCurrency(value as string | number | null)}
              </div>
            </div>
          ))}
        </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate mt-0.5">
                      {opp.pointOfContact
                        ...
                        : ""}
                      {opp.amount?.amountMicros
                        ? formatCurrency(
                            opp.amount.amountMicros,
                            opp.amount.currencyCode,
                          )
                        : "No value set"}
                    </p>
                  </div>
      ...
      {/* Upcoming Trips — persisted bookings */}
      {!operationalLoading && operational && (
        <div>
          ...
          {operational.upcomingTrips.length ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {operational.upcomingTrips.map((trip) => (
                <Link
                  key={trip.bookingId}
                  href={`/member/${trip.memberId}/trip-timeline`}
                >
                  <div className="lanai-card px-4 py-3 hover:border-primary/30 transition-all cursor-pointer">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      >
                        {trip.checkIn
                          ? new Date(trip.checkIn).toLocaleDateString("en-GB", {
                              day: "numeric",
                              month: "short",
                            })
                          : "Date pending"}
                      ...
                      <span className="text-xs text-muted-foreground">
                        {formatOperationalCurrency(trip.totalAmount)}
                      </span>
                    </div>
                    ...
                    <div className="text-xs text-muted-foreground truncate">
                      {trip.referenceNumber ?? `Booking #${trip.bookingId}`}
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <div className="lanai-card px-4 py-8 text-center text-sm text-muted-foreground">
              No confirmed upcoming trips in the platform booking ledger.
            </div>
          )}
        </div>
      )}
