  ...
  return (
    <div className="grid grid-cols-12 gap-2 items-start p-3 bg-muted/20 rounded-lg">
      <div className="col-span-2">
        <Select
          value={item.itemType}
          onValueChange={(v) => onChange({ ...item, itemType: v })}
        >
...
function CreateInvoiceDialog({
  ...
  type: "client_service" | "commission";
  onCreated: () => void;
}) {
  ...
  const handleCreate = () => {
    if (
      ...
      toast.error(
        "Add at least one complete line item with a valid quantity and price.",
      );
      return;
    }
    if (type === "client_service" && !memberId) {
      toast.error("Select the client to invoice.");
      return;
    }
    if (type === "commission" && !supplierId) {
      toast.error("Select the supplier to reconcile.");
      return;
    }
    if (type === "client_service") {
      ...
      createCommission.mutate({
        ...
        lineItems: lineItems.map((li) => ({
          ...li,
          commissionRate: li.commissionRate || "10",
        })),
        currency,
        notes: notes || undefined,
        dueDate: dueDate || undefined,
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button
              ...
              style={{ background: "oklch(0.35 0.09 145)" }}
            >
              {createClient.isPending || createCommission.isPending
                ? "Creating..."
                : "Create Invoice"}
            </Button>
          </div>
...
function CommissionReconciliationDialog({
  onGenerated,
}: {
  onGenerated: () => void;
}) {
  ...
  const generate = trpc.invoicing.generateCommissionReconciliation.useMutation({
    onSuccess: (result) => {
      toast.success(
        result.created.length
          ? `${result.created.length} supplier reconciliation invoice${result.created.length === 1 ? "" : "s"} created.`
          : "No eligible booking commissions were found for this period.",
      );
      setOpen(false);
      onGenerated();
    },
    onError: (error) =>
      toast.error(
        error.message || "Unable to generate the reconciliation batch.",
      ),
  });
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="gap-2">
          <Building2 className="w-4 h-4" /> Month-end Reconciliation
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle style={{ fontFamily: "'Playfair Display', serif" }}>
            Generate Supplier Commission Reconciliation
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 pt-2">
          <p className="text-sm text-muted-foreground">
            Creates one draft commission invoice per supplier from confirmed or
            paid bookings that checked out in the selected month. Existing
            supplier-period invoices are never duplicated.
          </p>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1.5 block">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              onClick={() =>
                generate.mutate({ month, dueDays: Number(dueDays) || 14 })
              }
              className="text-white"
              style={{ background: "oklch(0.35 0.09 145)" }}
            >
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Client invoices and supplier commission reconciliation
          </p>
        </div>
        <div className="flex flex-wrap justify-end gap-2">
          <CommissionReconciliationDialog
            onGenerated={() => refetchCommission()}
          />
          <CreateInvoiceDialog
            type="client_service"
            onCreated={() => refetchClient()}
          />
          <CreateInvoiceDialog
            type="commission"
            onCreated={() => refetchCommission()}
          />
        </div>
      </div>
                    ...
                    {(
                      ...
                      >
                        ...
                        <div className="font-semibold font-mono">
                          £{parseFloat(li.totalPrice).toLocaleString()}
                        </div>
                      </div>
                    ))}
                  </div>
